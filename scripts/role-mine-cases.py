...
**The `absent` case has a two-day hole in its corpus.** Between 2026-08-28 and
2026-08-30 a search result was rendered as title and address only, so a log
written in that window has no snippet line, `figures_only_in_snippets` finds
nothing in it, and no `absent` assertion is written from it — which is the case
type this script exists for. `case_for` still returns a `rows`/`distinct` case
for such a set; that is an extraction case over an input with no snippet in it,
and a weaker thing. Logs written before and after that window carry the summary
line and are fully minable. `docs/roles.md`, *Title and address only*.
