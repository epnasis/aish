...
class SessionLog:
    ...
    def _parse(path: Path) -> ParsedLog:
        ...
        that title was auto-generated, and the session origin ("user" unless a
        `kind:"origin"` record says otherwise — so triggered sessions cold-load
        with their provenance intact, #160). The `kind:"title"`/`"origin"`
        ...
        return ParsedLog(messages, model, custom_title, origin, title_auto)
    ...
    def load_entries(state_dir: Path, exclude: set | None = None) -> list["SessionEntry"]:
        ...
        for path in SessionLog._by_recency(state_dir):
            if path in exclude:
                continue
            messages, model, custom_title, origin, _ = SessionLog._parse(path)
            if not messages:
                continue
            content_cf = " ".join(
