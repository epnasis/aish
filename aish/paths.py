...
one function is what keeps a site from opting out by omission. The last three
sites that spelled their own default — the sign-in replay store, the skill
quarantine and the curate pass — joined under #399; before that a run which
believed it was isolated still read the owner's real recorded sign-ins.
`tests/test_paths.py::TestEveryConsumerAnswersTheSame` is the roll-call.
