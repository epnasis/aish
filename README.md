...
| `browse` / `browse_act` | Open a page in your signed-in browser and get a **numbered list of its controls**, then press one — for what lives behind a button rather than a URL | asks once per site per task, then again by name for anything that pays, deletes or cancels; **passwords are never typed** |
