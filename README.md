...
| `browse` / `browse_act` / `browse_fill` | Open a page in your signed-in browser and get a **list of its controls by name**, then press one by that name — `browse_act(target="Szukaj")` — for what lives behind a button rather than a URL. An action reports **what is new** on the page rather than the whole page again — what it does not list is what you last saw, and a click that changed nothing says so; pages arrive tiled into **named sections** (a section already shown collapses to one line on later pages, and the model can ask for a single section — or just the section index — instead of re-reading the page); icon-only buttons are described in words, and only controls that could actually be pressed are listed (anything shut in a menu, an off-screen panel or behind a dialog is counted, so aish looks for the opener instead of guessing a URL). On a list of results every row's button is named by its row — `Wybierz — 07:45 – 09:10 | LO123 | 640 PLN` — so you can see which one it is about to press. `browse_fill` does a whole form in one go — origin, destination, dates, then Search — picking the right suggestion when a field opens a list, and reporting what each control ended up holding. A file a click downloads is saved and can be read | asks once per site — **at the first press that actually changes something**, not the first press. Switching a tab, typing, choosing an option, following a link and pressing a plain search button ask nothing and grant nothing; the card arrives on the submit or the button whose name says it sends, saves or signs, and it is **one card, not two**: it names that press and grants the site together, so you answer once and a no refuses both. The same yes covers `read_url` there. The cost of moving it, stated plainly: a button that changes something but says nothing about it and submits no form — an icon-only trash can — is now pressed with no card where being the first press on a site would once have drawn one. That protection was accidental and depended on click order; what still stands is the refusal below, the values aish will never type, and the fact that every press is echoed as it happens. A control whose **own words** say it buys, pays, orders, books, subscribes, ends a contract or deletes has **no yes button at all**: aish refuses it, tells you what is on the page, and hands you `/browser <host>` to press that last step yourself. That is a promise about what a control SAYS — a nondescript "Dalej" that completes a purchase is not covered by it, which is why the grant is per site and every press is echoed as it happens. **Passwords are never typed**, and neither is a bank account number or a payment card — those are read off the **value** aish is about to send, so no field label, placeholder or page language can talk its way past them. Between *never* and *free* there is one middle tier, and it is the one you arm yourself: values you **declare** as your own — `aish personal set home_address` — draw **one card** before aish types one into a form, saying what it is and which site, *"about to type your home address on lot.com."* One yes covers that kind of value at that site for the rest of the task. It matches what aish is about to **send**, across the whole form, so an address split over street, postcode and city still counts as your address — in any order, and with other fields typed in between. Declare each value the way a site takes it in one field; aish finds it however the form is laid out. The same card comes up if one of them ends up in an address aish builds or in a web search, at **any** site, including ones you have already agreed to send data to and whether or not aish has read anything on the web this turn. It is a card and not a refusal because filling in a shipping form is sometimes exactly the job; in an unattended session it is a refusal, because nobody is there to read a card. If the Keychain will not hand the values over, aish **stops rather than guesses** — it says it could not read them, and that it does not know why. `aish personal list` / `rm` manage the set (the listing says which values are actually covered), the values live in the macOS Keychain in their own namespace (no plugin tool can name one, and `aish secret get` cannot reach one), and **the card's sentence names the kind of value and the site, never the value**; a form card shows `[your home address]` where the value would be. That mask is on the SENTENCE. The **arguments** of the call carry what aish typed, and they appear beneath the card, in the approval line written to the session log, and in the trace — as every other tool's arguments do, and on purpose: an audit record that hid what was sent would be worth less than one that shows it. Two more things it does not cover, said plainly: aish matches only what you have declared, so a CVV, a one-time code or a national ID are values it will type; and those arguments are written to the session log before any gate runs, exactly as they are for a refused card number. And if aish cannot read the list of declared values at all — a bad file, a locked Keychain — it **refuses to type anything into any form** until it can, and says it does not know why. Opening a page asks the same address question `read_url` asks, and gets the same answer — and so does SENDING a form: pressing a submit that carries what aish typed, at a host you have never agreed to send anything to, asks once for that host and shows you the values it would send. The host is **where the form sends**, which is not always the site you are looking at — a page can carry a form that posts somewhere else entirely, and that draws the card even on a site you have already agreed to. Where aish cannot read where a form goes at all, it asks and says so rather than assuming. Filling in and sending an ordinary form — one that posts back to the site it is on, at a host you have already agreed to — asks nothing |
| `web_search` / `read_url` | **Two search indexes at once** — DuckDuckGo, and Google's own results page read in a browser signed into nothing — merged, one result per page; then fetch a page as readable text; a bot-blocked or JavaScript-only page is re-read in a **real browser** on your machine, and a site you're signed into is read **through your signed-in session** from the start. A search result reaches the assistant as its **title, link and the index's own summary line**, behind a banner naming which of those words a stranger wrote; every one of them is capped and stripped so it cannot fake a result or wear aish's own voice, and the assistant is told to take a figure only from a page it actually opened — which is an instruction to it, not something the code enforces | auto; every query/URL echoed; public hosts only (SSRF-guarded); **reading a site you're signed into is free**; what asks once for the site is the first press there that changes something, and that one yes covers reading it too. Once a turn has read the open web, an address aish *composed* asks once more — and the card says what it found in that address (a stored secret of yours, a second address inside it, a run of random-looking text, or a query going somewhere you haven't agreed to), never a guess about what it is for. That yes is per host — the exact host the card named, not its subdomains — and it is asked **once, ever**: it survives aish restarting, the next chat, and the terminal as well as the web. The hosts you have already approved in earlier chats are carried over the first time this runs |
...
export AISH_LOCAL_SAMPLING='{"temperature": 1.0, "top_p": 0.95, "top_k": 20, "presence_penalty": 1.5}'
                                     # sampling sent on every request (these are the defaults)
...
mlx-lm stops at 512 tokens when a request leaves it out. Sampling is always
sent too, because mlx-lm decodes greedily (temperature 0) unless the request
says otherwise, and has no server flag for `presence_penalty`; the defaults
...
(`enable_thinking`). The OpenAI variables (`OPENAI_API_KEY`, `OPENAI_BASE_URL`) are never read for `local:`,
...
> can't land in a file. Nor are the personal values you declare with
> `aish personal set` — those are in the Keychain too, in a namespace of their
> own. What the card's *sentence* names is the value's *kind* ("home address")
> and the site, and a form card shows `[your home address]` where the value
> would be — but the **arguments of the call carry what aish typed**, and they
> appear in the card's argument block, in the approval line written to the
> session log, and in the trace, exactly as every other tool's arguments do.
> That is deliberate: an audit record that hid what was sent would be worth
