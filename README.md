...
| `browse` / `browse_act` | Open a page in your signed-in browser and get a **list of its controls by name**, then press one by that name — `browse_act(target="Szukaj")` — for what lives behind a button rather than a URL. An action reports **what changed** on the page rather than the whole page again, so a click that changed nothing says so; icon-only buttons are described in words, and only controls that could actually be pressed are listed (anything shut in a menu, an off-screen panel or behind a dialog is counted, so aish looks for the opener instead of guessing a URL). A file a click downloads is saved and can be read | asks once per site per task, then again by name for anything that pays, deletes or cancels; **passwords are never typed** |
