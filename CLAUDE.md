...
**The same rule, one store over: a suite run must leave `~/.config/aish` byte-identical.** `isolated_global_dirs` redirects the knowledge dirs by rebinding constants each module bound at IMPORT, so any path that captured one earlier writes to the owner's real corpus and nothing says so. On 2026-08-24 that happened: 49 fixture memories (`f000`–`f019`, `rule-000`–`rule-021`, …) landed in `~/.config/aish/memory`, the WatchPaths autocommit pushed them to the backup within seconds, and they were live entries for three weeks until a retrieval probe returned one. `no_writes_to_the_real_corpus` in `tests/conftest.py` compares a session-start listing against a session-end one. The state dir is deliberately NOT guarded — the owner's aish-web appends to it continuously, and a guard that fails when he uses his own agent is one people learn to ignore (#381).
