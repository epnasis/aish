...
- **`session.py`** — append-only JSONL per session in `~/.local/state/aish/`: conversation (for `--resume`), audit trail of every command decision, plus `kind:"trace"` records — the structured activity-trace steps (the same dicts the web renders), logged from BOTH entry points via the agent's `step_log` sink so the trace is UI-agnostic and survives eviction/restart. `reconstruct_events()` groups a logged session back into the `user`/`step`/`done` event stream a rich client replays (one `done` per task, carrying the final assistant text), so a cold-loaded web session rebuilds the SAME collapsed timeline a live one shows; it returns `None` for pre-trace logs, and the server falls back to a flat `history` blob then. `step_log` is orthogonal to the agent's `on_step` (rich-renderer hook that gates terminal chatter via `_note`) — the CLI wires `step_log` only, so it logs steps without changing its inline output. Its ranked excerpt search (`search_excerpts`, output hard-capped) backs the sessions section of the `recall` tool; the agent gets `state_dir` + `current_session` at construction and excludes the live session from results.
