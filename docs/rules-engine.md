...
**The model going silent is a third way out of the ask loop, and it ships the answer too.** On 2026-09-23 (`session-20260923-212625-506203`) a rule rejected a full answer, the model opened one of the three links it was sent to open, then returned nothing twice — and the owner got a placeholder over an answer that existed, which turns *restrict* into *confiscate*. So the last rejected draft of the task (`_last_rejected`) is delivered when a turn would otherwise end with no answer text — from the loop and from `_finish_stopped` alike — prefixed by `REJECTED_DRAFT_DELIVERED`, which states only what was observed. It is checked again at delivery, never asked about (asking re-enters the loop that just produced nothing), so its note reflects the evidence **as it stands then** — a link opened after the rejection is not named. The trigger is *no answer text*, never *a shorter answer*. It adds no exposure class: an answer that failed its check already ships with a note once asks run out. Rejecting also clears the held entry, which fixed a real leak found in review and reproduced: a turn stopped by the loop detector whose wrap-up call failed logged the rejected draft whole, with no note, while the live stream showed only the stop headline. `TestARejectedDraftIsNeverLost`.
...
**A rejected draft must be REPLACED, and the model has to be told so.** The draft stays in the model's own history (the ask refers to it), so from the model's side it was said — and *"add it and give the answer again"* read as *"send the missing part"*: on 2026-09-23 (`session-20260923-214217-425102`) a rule asking for tap buttons got back the buttons alone, Verify passed them, and the email list the draft carried was lost. So when the rejected answer was held, the agent appends `ANSWER_WITHHELD` to the ask — the owner has not seen it, give the complete answer again. It is the agent's sentence, not a template in `rules.py`, because only the agent knows the draft was held. **No check backs it**: a "the redo is much shorter" test was rejected in review as an uncalibratable threshold that a correct short answer fails and a padded wrong one passes, and gluing draft and redo together would re-insert text an `answer_must_not_include` rule removed. If fragments recur, the next step is the experiment of dropping the rejected draft from the model's history — measured, never a length heuristic. `test_a_rejection_tells_the_model_the_user_never_saw_the_draft`.
...
**Binding is not firing, and counting only binds made the ledger unreadable.** Three trigger kinds bind at seed and decide later — `action:` at the gate, `answer:` at turn end, `result:` when a result lands — so for them a bind means *it is watching* and the bind count is a constant, one per turn. Measured on the live corpus: **17 of 23 rules reported `binds on 100% of turns — narrow the trigger`**, every one of them advising the owner to narrow a trigger that binds by construction and has nothing to narrow. Both existing signals were pointed at the wrong number. The one that catches an over-broad pattern cannot see one here; the one that catches a dead rule (`never bound in N turns`) is blind to this entire family, because they always bind — `live-price` read `binds=50, refusals=0` and looked healthy while the log said it had checked a price a handful of times. `applied` is the count either signal was actually asking for, and the gate records already carry it: `evidence.applied` on an answer rule's verify row, a non-`allowed` verdict, or `armed.fired` for a latched `result:`. One reader for all three, because a caller that had to know which trigger records its firing where would be re-deriving the engine from source. **A turn contributes at most one `applied`** — a rule that refused four calls governed one turn, and per-row counting would make a chatty model look like a busier rule.

**A broken file needs a WHEN, not a count.** `29 turns could not compile it — broken file` reads identically whether the breakage was this morning or nine days ago and long since repaired, and *whether to go and open the file* is the entire decision the line exists to inform. The live corpus was the second case: every `error` row in the window came from the grammar migration passing through in early August (`trigger:` retired, then `request:` → `prompt:`, then a rule caught mid-rewrite with a `when:` and no `then:`), and both affected files had been clean on every turn since — while the ledger went on reporting them in the present tense for the rest of its fortnight. So `RuleStats` carries `last_error` and `last_clean`, and it is their **order** that decides: a rule that compiled last week and broke today has both stamps, and reading the mere presence of a clean run as "fixed" would be the worst available reading of the pair. The ledger also carries `scanned_at`, so "3 days ago" is relative to the window the counts came from rather than to whenever a caller formats them — the alternative was threading a clock through every call site, which is a footgun the first test of it duly fell into.

Two signals follow from it, and the second is the one that needed an argument. A rule that binds and never applies is flagged — but **only if it DEMANDS something** (`must_first`, `answer_from`, `answer_must_include`). A `never_use` that nothing ever trips is the rule **deterring**, which is the rule working; flagging it as dead weight would aim the ledger squarely at the rules earning their keep silently. That distinction is why the ledger snapshots the compiled `verbs` off the binding rather than reading today's rule file — corollary 1 again. And the broad-bind signal now skips the structural triggers (the arming three, plus `always`), because *"narrow the trigger"* is not advice about a rule that has none.
...
**`split_frontmatter` lives in `skills.py` and rules.py imports it (#209).** Rules are the fourth artifact class in one md+frontmatter family, and they already inherit that family's lifecycle from `skills.lifecycle_active` for exactly this reason — a second reading of "where does the header end" is a second thing to keep in step. `rules.split_frontmatter` still resolves; only the definition moved. The rationale now sits beside the function in `docs/knowledge-layer.md`.
...
**And the rule built on it was retired (#212), then un-retired — the useful part is what the measurement showed.** `answer-me-first` (`when: always` + `must_first: answer`) was retired on the argument that narration makes it unnecessary *by construction*. That has it backwards. Six controlled runs across three independent replications, the rule as the only variable, show **perfect separation on whether the model speaks at all: 30/26/57 narrated turns with the rule on, 0/0/0 with it off.** The rule PRODUCES the words; narration is the layer that displays them. Retire it and aish goes silent — which is the spinner this whole issue was filed about.

**Two claims made along the way were wrong and are retracted here rather than quietly dropped.** That the rule also made tasks *faster*: the same runs give **30/26/57 turns on (mean 37.7)** against **33/32/29 off (mean 31.3)**, direction reversing between replications. And, before that, that a system-prompt paragraph caused a slowdown. Both came from comparing single field runs of a task whose step count varies **26 to 57 under identical settings**. The transferable rule is not about this rule at all: **on a task with that much run-to-run variance, no single-run before/after comparison can support a causal claim** — and both wrong answers were produced by exactly that. Retiring a rule still needs its replacement observed, which is how the narration half got settled. `TestAnswerBeforeActing`, `TestAnswerFirstGate`.
...
**Identity is the page, not the string** (`normalise_url`): fragment and trailing slash go, and a playable video collapses to `video:<id>`. The second one is the first one's argument taken a step further — `youtu.be/ID`, `watch?v=ID` and the `&si=…` the phone's share sheet appends are one video. It is load-bearing, not tidiness: the owner shares the phone's form, the answer shows the canonical form (`show_image` composes a video still's card from the id — #217), and compared as strings the rule reported *the one video it was certain he had opened* as a link nobody opened. The collapse is to the ID and nowhere wider — a channel or playlist URL keeps its ordinary identity, or one video would verify a link to the whole channel. `TestLinksYouDidNotOpen`.
...
#### The ledger is the CHAT's, not the turn's

**"Every link must be one aish opened *this turn*" was one word too narrow, and the word was "turn" (#267).** A follow-up question repeats the links of the answer before it, the rule refuses them again, and the model dutifully re-reads pages that are already sitting in the transcript. Measured over every session log on the owner's machine: **130 firings of `unverified_links`, 53 of them naming a URL a successful call had already opened in an earlier turn of the same chat — and 42 (32%) where EVERY refused link had been, within the day.** Those 42 are firings the ledger silences outright; the other 11 mixed an old link with a genuinely new one and still fire, correctly, with less to chase. One chat did it twelve times; another refused the same Decathlon carabiner URL on turns 8, 9 and 10. Each costs a refused answer, an ask round, a repeat fetch and another model turn — the rule taxing an answer for being consistent with the one before it.

**Opening a page is a fact about the fetch, and the fetch does not un-happen when the model stops talking.** So `_acted_on` is now the union of what ran this turn and what this chat has opened before it — `TurnEvidence.opened_before`, filled by the agent from the same `rules.urls_acted_on` the check itself uses. One definition of "opened", or the ledger and the check would disagree about the same call.

**This is not the widening #249 was closed for refusing.** That asked for URLs the harness merely *showed* the model — read off a rendered page, never fetched — and the answer was no, because *a link you can open is better evidence than a link you saw*. These URLs were opened, by aish, successfully. Only the turn boundary made them stop counting, and a turn boundary is not evidence about a URL.

**Provenance is durable; freshness is not, and the corpus already splits them.** `unverified_links` is the provenance rule — *you did not invent this and it loaded*. `unverified_prices` and `availability-is-checked` are the freshness rules, and both stay strictly turn-scoped: `_pages_read` deliberately does not read the ledger, so a figure still has to come off a page fetched in the turn that prints it. Widening the price join the same way would re-create the exact bug `live-price` was written from.

**A horizon, because one of those two properties does decay.** `OPENED_LINK_TTL` = 24 hours: inside a day it is one fetch either way, and past it a chat reopened next week would be vouching for a link nobody has touched since — the rule quietly not doing its job, which is the failure mode R1 says a rule engine may not have. Expiry is evaluated when Verify READS the ledger, not pruned on write: a chat can sit idle between turns, and a ledger that only ages when something happens would hand the next turn a set that expired days ago. `OPENED_LINKS_MAX` = 500 bounds it, oldest out first — a memory bound, not a policy, and a trimmed URL costs one re-read.

**It has to survive the agent that recorded it.** A chat gets a fresh agent every time it is reopened, which on the web is every restart of `aish-web`, i.e. every ship — so an in-memory ledger alone would go turn-scoped again for exactly the chats with the most history to reuse. `Agent.restore_opened_links` refills it from `SessionLog.calls_that_ran`, the same shape as `resume_turns` beside it (`docs/session-log.md`). A call with no readable timestamp is dropped rather than dated now: a URL of unknown age arriving fresher than today's reads is the one way this could vouch for something it should have re-checked. `TestTheChatsOpenedLinks`.
...
### false_access_claims — the check that was withdrawn

**There was a third named join — the vocabulary word was false_access_claims, and both it and its check are gone. Do not rebuild it in this shape.**

The rule was `read-means-read`: *"never tell me a site blocked you, in a turn where you read that site."* It was written from a real and expensive failure — a run that reported a `403 Forbidden` from Allegro in a turn where all twelve of its reads succeeded, then abandoned the shop the owner had asked for and answered from Decathlon without saying why. The claim is an **exit**, not merely a wrong sentence, which is what justified spending a check on it.

The check was a lexical marker list (`403`, `forbidden`, `blokad`, `zabezpiecze`, …) attributed to the nearest host in the sentence, intersected with the hosts the turn had read. In production it fired **9 times across 5 sessions in 734 logs, and was wrong every time**. Every firing spent both ask rounds and then stapled a false accusation onto a correct answer, which the owner read.

Two independent causes, and the second is the one that generalises:

- **The claim side is lexically undecidable.** `403` matched inside a LinkedIn job id (`…-at-google-4283040326`), and `google` in the same slug won the nearest-host attribution — so a Google Careers answer was accused of claiming Google had blocked it. Separately, an answer about a **blokada antykradzieżowa** — a motorcycle anti-theft *lock* — tripped `blokad` and `zabezpiecze` three times, because in Polish the block-words are ordinary shop vocabulary. Stripping URLs fixes the first and not the second. The founding write-up already noted that *"the only 403s anywhere in that log are digits inside offer ids"*, and a substring match shipped anyway.
- **The evidence side counted walls as reads.** The helper behind it built "hosts successfully read" from the *arguments* of any call that was not refused; it never looked at the result, and `_ran` — which is still there — only asks `status != FAILED`. A 200-response login wall, an empty JavaScript shell and a `404` all counted as *"you read that site perfectly well."* So the eon.pl and claude.ai firings were the harness handing the model a wall and then refusing it for describing the wall — the same compounding recorded in `docs/browser.md`.

The shipped comment defended the marker list by arguing it **under**-fires and that this is the safe direction. Five out of five say it over-fires. That is the durable lesson and it is why this section survives the code: **R1 says a rule-engine bug can only over-restrict, and over-restriction is therefore the only failure mode a verify check has — so "it can only under-fire" is never the argument that makes one safe to ship.** A named join needs its false-positive rate measured against real logs before it earns `when: always`.

If the behaviour recurs and is worth chasing again, the two things this attempt got wrong are the two places to start: count a read as a read only when it returned substantive content (the harness already classifies this — `say-when-the-link-failed` arms on `was: empty`), and put the claim side on the meaning tier rather than a substring list, since register is what similarity measures and the marker list is a translation bill that never ends.
...
**Seed** (`Agent.seed_rules`) runs at the top of every task, from the same position the knowledge step is emitted — before the user message, so nothing this turn dispatches can outrun it. It is called by **both** entry points: `run_task`, and `ClaudeMaxAgent.run_task`, whose SDK owns its own loop. A rule that governed only local turns would be a rule the model escapes by being asked on a different backend. The prose rides the per-task system reminder on the native loop. Earlier reminders stay in history (append-only, for the prompt cache — `docs/agent-core.md`), so a later reminder declares them superseded and states this turn's rules in full, as `RULES_UNCHANGED`, or as `RULES_NONE`; the gate binds per turn whatever old prose remains readable, and since rules only restrict, stale prose can only over-restrict. Under claude-max it rides the prompt itself; `mark_rules_seeded` writes the `binding` records only once it has landed. `TestRuleSeeding`.
