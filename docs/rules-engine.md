...
**And the rule built on it is retired (#212), while the verb stays.** `answer-me-first` (`when: always` + `must_first: answer`) was written to paper over the silence, and it papered badly: it made the model cautious, and it never checked what its name claimed — a one-word preamble satisfies *"was any text produced before the first tool call"*, which the honest limit above already admitted. Now that the model talks as it goes, the rule is unnecessary **by construction**, and the case it was written for — a denial must be answered before anything else runs — is enforced in Python by the stop gate, not by advice. The verb is not retired with it: `must_first: answer` remains the only legal non-tool value, it compiles, and it is the worked example for an ordering the gate decides. This is the corpus shrinking because the harness got better, which is the direction the lifecycle is supposed to run in.
