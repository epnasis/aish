...
The store is `put_personal` / `get_personal` / `delete_personal` / `personal_names` / `personal_matches`, with a NAME index of its own in the state dir and its own cache and TTL (the gate asks it of every value it is about to type, and a Keychain read is a subprocess). `put_personal` **refuses** a value shorter than `MIN_PERSONAL_MATCH = 6` folded characters, naming the floor and the count, because a stored-but-unmatchable class is a fence he believes he has and does not; `personal_matches` skips a short value anyway, so a value arriving by any other route can never widen the match to everything; and `aish personal list` marks such a class as NOT fenced so the listing cannot claim coverage the matcher does not give.

**A name the index lists and the Keychain will not hand over is UNREADABLE, and it is a THIRD state.** `get_personal` answers None for *not stored* and for *could not read*, so a locked Keychain or a refused TCC prompt folded to "too short" — the value was typed free, with no card and no record, and the listing printed a cause nothing checked. `personal_unreadable` tracks those names apart and `personal_status` answers `fenced` / `short` / `unreadable`; the browse gate fails closed on them on all four paths (→ `docs/browser.md`) and the listing says what actually happens — *aish will not type into ANY form until it can* — rather than "not fenced", which was the opposite of the gate's behaviour. `_read_index` catches `ValueError` as well as `OSError`, since a non-UTF-8 index raises `UnicodeDecodeError` and that used to propagate through both gates to `_dispatch`'s generic handler: contained by accident, which is not a failure direction. An exception refreshing the cache answers with every declared name, because not knowing is not the same as knowing there is nothing.

`aish personal set` reads the value **twice**, and that is not ceremony: a mistyped API token fails loudly the first time it is used, while a mistyped home address fails SILENTLY — the fence simply never matches and he keeps a class he believes is covered. It also prints the one instruction that decides whether the fence can see the value at all: **declare it the way a site takes it in ONE field** — the street line on its own, the phone without the country code. The suite-wide guard extends once more — `no_real_secrets` also redirects `personal_names_index` and invalidates the second cache — for the same reason it covers the first two: without it a suite run shells out to `security` for the developer's home address to decide that a fixture's search term is not it. The rationale for the fence itself lives in → `docs/browser.md`.

**Both name indexes resolve `AISH_STATE_DIR` at CALL time (#353), and that is a safety property rather than tidiness.** They were `Path.home() / ".local" / "state" / "aish" / …` bound at import, while `vouches.state_dir`, `browser.state_dir` and `explain.state_dir` have all resolved it at call time all along — `server.py`'s own comment even claimed `secrets` did. So a process that had set `AISH_STATE_DIR` to a sandbox still read and **wrote** the owner's real index: it defeated the isolation of a probe run against the declared-value slice, which wrote into his live state directory believing it was sandboxed. `secrets.state_dir` / `names_index` / `personal_names_index`.

**And an index that EXISTS and cannot be read is never answered as an empty one (#353).** `_read_index` returned `[]` for every `OSError` and `ValueError`, so a bad mode or corrupt bytes were indistinguishable from a machine where nothing had ever been declared — `personal_names()` and `personal_unreadable()` both empty, the fence silently OFF on every channel, and `aish personal list` printing nothing declared. Reproduced at `chmod 000` before it changed. It now returns `[]` only for `FileNotFoundError` and raises otherwise; `personal_unreadable()` is the reader every gate asks, it never raises, and in this fault it answers `INDEX_UNREADABLE` — the LIST, not a class, because aish does not know what the classes are and naming one would be a cause no line checked. The listing catches it and says the index could not be read, with the path and the error, because aish does not know why.

**`names()` deliberately does NOT do this, and the gap is stated rather than left to be found.** The stored-secret list feeds `scrub`, which runs over every tool result and every console line, so raising there turns a file-permission glitch into a failed turn on every call — and `scrub` degrades REDACTION where `personal_names` decides a PERMISSION. But `contains` reads the same list and IS a gate input (arm 3 of `_value_finding`, the stored-secret arm of `_driven_finding`, `_command_has_a_secret`), so an unreadable secret index still disarms those arms silently. That is the same fault one namespace over, with a blast radius that makes the same fix wrong. **It is not fixed and is not claimed to be.**
