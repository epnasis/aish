...
Conflicting arguments **error rather than resolving to a winner** (`at=` with `chapter=`; `count=` without `every=` and `every=` without `count=`; a `count=`/`every=` series with neither `at=` nor `chapter=` to start from; and `search=` beside any of `at=`, `chapter=`, `duration=`, `count=`, `every=`): a model that wrote both did not know which it meant, and silently honouring one returns frames from a place nobody asked about, cited as though they were. Three of those shipped as silent drops rather than errors — `every=` alone yielded ONE frame with the step ignored, a series with no anchor fell into the opening-frame branch which discarded both parameters, and a series beside `search=` was ignored by the search path — each a request honoured in name only, which is the same silence as a wrong place (#216). The rule is one rule, not a per-site patch: an argument that was written and not honoured is an error, whichever branch would have dropped it. The opening frame is deliberately not second zero — a video's first moment is routinely black, a title card or a logo, and a blank picture reads as "nothing to see" rather than "you looked too early".
...
**A rendition carries where it came from, beside the bytes (#319).** Being inside `workspace_roots()` is what makes the feature work and is also what made the store a door nothing asked at: a caption track is written by whoever uploaded the video, and read back through `read_file` it arrived bannerless, untainted and unattributed — a perfectly good place to put "ignore previous instructions", landing in context looking like a file the owner owns. The door stays open, because grepping the file the tool just named IS the feature; `load_transcript` writes a `provenance.record_artefact` sidecar beside every rendition instead, and the file layer marks and taints the read from it (`docs/agent-core.md`). Unconditional and with no parameter, because there is nothing to decide — a caption track is always fetched. Rewritten on a cache hit too, so a rendition whose record was lost does not stay unattributed for as long as it lives. `prune_transcripts` skips records when counting and unlinks them with the rendition they describe: a record is part of its artefact, never an entry of its own. `TestTranscriptStore`.
