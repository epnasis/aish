...
**A provider that missed the wait is absent THAT time, not for the day (#412).** The cache is written whenever any provider answered, so a single slow reply — the owner's MLX server is on another box — used to be cached as "this provider has no models" for the whole `CATALOG_TTL`. A configured provider the cache lacks is now asked again, alone and merged into the rest, at most every `CATALOG_RETRY` (10 min): often enough to recover, rarely enough that a server that is really down does not cost every picker open the full wait. The day's clock runs from the full fetch; a retry moves only `checked`. `test_a_provider_that_missed_the_wait_is_not_hidden_for_the_day`.

**The web picker's Recent is the models the owner's chats RAN on (#412)**, read off their `model` records by `SessionLog.models_used` (user chats newest first, within a chat the last-used model first; triggered chats excluded — their model is config's choice) and filtered by `recent_models`. It was a per-device `localStorage` list written only by a tap on a picker row, so a model used on the other device, inherited by a new chat, restored by reopening a chat or chosen in the terminal was never Recent — reproduced with two browser contexts: the MLX model used on one, listed in the second's `model_list`, absent from its Recent. Server-derived, it is one fact on every device. "Used" means a `model` record exists, and `model()` writes one lazily, just before the chat's next record of any other kind: a model selected and followed by nothing is not Recent, one followed only by a rename is. What `recent_models` drops: the current model; claude-max (the web cannot switch to it without a restart); an Ollama name that is not listed (gone, or Ollama down); and any provider model whose provider is not configured (`_configured_providers`, the same env gates `make_chat` enforces). A configured provider's id stays even when the catalog missed it this time — the catalog is the only way those ids reach the list, and `make_chat` builds the id either way; whether the provider still SERVES that id is not checked. A bare provider spec (`aish --model gemini` logs it raw) is spelled out as `provider:default`, so the same model never appears twice. `TestModelsUsed`, `test_recent_*`, `test_model_list_says_which_models_the_chats_ran_on`, `tests/js/test_model_recent.js`.
...
`load_config` reads `config.toml`; malformed config degrades to defaults rather than to nothing. `usage_context` builds the system-prompt section that describes aish to itself — **when user-visible behaviour changes, both the README and these strings need updating**, since aish answers questions about itself from them. It carries `IDENTITY_SLOT`, not a baked identity: `identity_context` lives in `agent.py` now and the agent fills the slot from its live model at every rebuild (`docs/agent-core.md`, *The identity section is filled from the LIVE model*), which is why `switch_model` no longer prints a "restart to refresh" note. `load_context_files` pulls in the project's own context files. `TestConfig`, `TestUsageContext`.
