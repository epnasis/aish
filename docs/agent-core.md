...
Narration (#212) is the SDK's to emit, so `on_delivered` stays on the wrapper beside `on_message`/`on_token` rather than being forwarded. A **later** assistant message is what proves the previous one was not the answer, so a delivery is closed one message late rather than guessed at from the presence of tool_use blocks; each is recorded like any assistant turn (a cold reload replays the narration instead of losing it) and joined to `inner._delivered`, so `verify_final` grades everything the owner was told. `TestNarration`.
