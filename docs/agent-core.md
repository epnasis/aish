...
So a turn now makes several **deliveries**. `_deliver_interim` closes each one: any turn whose prose came with tool calls is not the answer, so its words go out whole, as a message, the moment that step ends. The status line keeps its one capped line — the trace is the developer's view of *which tools ran*, and this is the human-language layer above it, not a replacement for it.
...
**What is deliberately NOT enforced.** Nothing checks that the model narrates, or that it narrates *well*. *"Only speak when you have something worth saying"* has no verdict function behind it, so writing it as a rule would be an unenforced verb — the exact thing the admission line in `docs/rules-engine.md` forbids. It lives in the system prompt (rule 4b) and is tuned against real turns. Noise is the standing risk and the only instrument for it is reading output.
