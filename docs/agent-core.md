...
**Nothing prompts for narration, and the first attempt to is why.** The mechanism delivers whatever the model says beside a tool call; no rule requires it, because *"only speak when you have something worth saying"* has no verdict function and would be the unenforced verb `docs/rules-engine.md` forbids. A system-prompt paragraph (rule `4b`) was the intended home for it and **shipped, regressed, and was withdrawn the same day.** Measured on the owner's own logs — same prompt, same model, same 17 bound rules — a task went from **21 turns to 50**, ~8 minutes and 1.87M input tokens, with **zero narration emitted**: every `say` on the turn was empty. It bought nothing and cost 2.4x the steps. 4b was the one and only model-visible change in the whole of #212, which is what makes the attribution clean.

Two lessons worth keeping. **A prompt paragraph is a behavioural change and needs a measurement, not a review** — the code was verified end to end in a browser and the prompt was verified by reading it. And the shape of the text was already flagged before it shipped: capability framing first (*"it is how they follow a long task"*), the constraint buried below it, two different trigger criteria, and quoted padding examples in the context of a model being asked not to pad. Any second attempt states silence as the default output, gives one criterion, quotes nothing — and lands only with a before/after turn count behind it.
