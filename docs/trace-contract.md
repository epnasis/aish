...
| `frame_skipped` | Why there is none, when there is none: `password` \| `unknown` \| `hands` \| `failed`. `password` names what was SEEN (a password box), never what the page WAS — the check is not a sign-in detector. `unknown` is a page that would not answer that check, and is deliberately not folded into `failed`: a capture that broke and a page that would not say what it was route to different repairs. A closed vocabulary, because two renderers read it back and free text would drift between them. Written only when `frame` is empty; a step carrying NEITHER is the third state — a log written before frames existed, or a tool with no page — and must never be reported as an empty capture (corollary 2). |
