...
| `signin` | **The automatic sign-in that happened INSIDE this call** — `{host, frame \| frame_skipped, console, covered}`. `covered` is the same fact as the key above, for the SIGN-IN page's own submit button, and is a bare string because the sign-in path has no consent-dismissal retry to report. Written by `browse` and `read_url` only, since renewal happens on the open and never on an act. It is deliberately NOT hung on `frame`: that key claims *the page the model was SHOWN*, and the model is never shown the sign-in page — it is a different document, on a page it did not ask for and cannot name, so borrowing the key would state a guarantee wider than the capture enforces. A key on the existing `tool` step rather than a new kind, because a kind with no renderer opens an empty live trace card (§1.2) and this needs none. `host` is what says an attempt happened: no `host`, no block — an empty one would read as an attempt with nothing to show (corollary 2). Its `frame_skipped` vocabulary and its console are the same as the fields above, under the same rules. |
