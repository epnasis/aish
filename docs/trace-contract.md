...
| `frame_skipped` | Why there is none, when there is none: `signin` \| `hands` \| `failed`. A closed vocabulary, because two renderers read it back and free text would drift between them. Written only when `frame` is empty; a step carrying NEITHER is the third state — a log written before frames existed, or a tool with no page — and must never be reported as an empty capture (corollary 2). |
