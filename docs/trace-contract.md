...
| `phases` | **Where a driven action's seconds went** (#348), in milliseconds — `settle_ms`, `enumerate_ms`, `frame_ms`, `act_ms`, the `bar_ms` the settle was held to, `controls`, `steps` and `stopped_at` on a batch, and a `settle` block (`waited_ms`, `polls`, `idle_ms`, `first_quiet_ms`, `last_arrival_ms`, `traffic_waited_ms`, `released`). Written by the browse tools only. `stopped_at` is the step a batch died on, which `steps` alone cannot say — a batch that died on step 2 of 6 and one that died on step 6 of 6 spent their time differently. `first_quiet_ms` is -1 when the first poll did not answer, never 0: an unknown written down as a zero would read as "the page had just moved", which is the opposite of what was seen. The step already carried `secs` for the WHOLE call, which is why *why is this slow?* was unanswerable: one flight-search task spent 185s across 18 browser actions and nothing said whether that was the settle bar, the enumeration, the screenshot or the page. **`released` is a closed vocabulary** — `quiet` | `timeout` | `unknown` | `torn` | `quiet_with_traffic` — for the reason `frame_skipped` is one: a wait that ended at its ceiling and a wait that ended because the page went still are different facts, and reading the first as the second is how a timeout comes to be reported as a finished page. **`last_arrival_ms` is a SHADOW measurement and the only field here with a decision hanging on it:** how long after the wait began the page last moved, so a log says per real page what a shorter release bar would have cost. -1 means the page never moved while it was watched, which is not the same fact as moving at 0ms. It is measured and never acted on — nothing reads any of this back to decide anything, and in particular the bar it would license must not be shortened by the wait that measured it. Every value is aish's own clock, so §8.6 is untouched. Absent on a call that never reached a page (corollary 2). |
