...
| a tool contract with no declared required fields | `required.declared: []` on the `tool` step | does not exist |
...
| `verify_pass`, `verify_fail`, `verify_unverified` | `gate{at:"verify"}` | `unverified` (judge unavailable) must be counted separately — a rule whose verification silently degrades to disclosure looks compliant. |
