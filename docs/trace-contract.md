...
| `messages[].digest` / `chars` | The content address of the message's JSON serialisation — **keys in the order sent**, no whitespace, non-ASCII unescaped (`agent._as_sent`; sorted keys via `agent._canonical` on a record without `order`, see the #420 note at the top) — in the chat's directory, and its length. Never sorted: key order is part of the prompt for a server whose template renders the request as JSON text. `chars` is of the serialised message, which is what crossed the wire; it therefore counts a tool call's arguments and `raw_blocks`, which `len(content)` never did. |
