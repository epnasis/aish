...
| `messages[].digest` / `chars` | The content address of the message's canonical JSON serialisation (sorted keys, no whitespace, non-ASCII unescaped — `agent._canonical`) in the chat's directory, and its length. `chars` is of the serialised message, which is what crossed the wire; it therefore counts a tool call's arguments and `raw_blocks`, which `len(content)` never did. |
...
| `options` | Every other keyword the client was handed — `max_tokens`, `cache_control`, `thinking`, `extra_body`, ollama's `options`/`think` — round-tripped through the canonical form. `stream` and `stream_options` are excluded: they decide how the answer arrives, not what was asked. |
| `request` / `chars` | The digest and length of the canonical serialisation of the whole stored payload: `{**options, messages, tools?, system?}` with the stored (placeholdered, scrubbed) messages. **A reassembly from the manifest is checked byte-identical against what the adapter sent**, on every backend with a converter (`TestSentAtTheSeam`) and on the pass-through shape end to end (`TestSentRecord`). `chars` is the one integer that makes the two ints proposed on `reasoning` in `docs/token-accounting.md` redundant. |
