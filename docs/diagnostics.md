...
- **`signin`** — the automatic sign-in that happened inside the call, on a page of its own: host, its own picture under the same three states, and its own console. Kept apart from `frame` on purpose, because the model is never shown the sign-in page and a reader that folded the two together would be claiming it was. It is rendered as *aish ATTEMPTED an automatic sign-in* by both readers, and that is the reader law rather than a wording choice: the block's `host` is written whenever an attempt happened, success or failure, so the line it used to carry — *aish signed in again at eon.pl* — asserted an outcome no field in the record holds. The outcome lives on `signin.Record` and in the note the model was given; a dossier may not re-derive it from an attempt's presence. **Two keys inside that block are RECORDED and not yet read here** (#325): `verdict`, the token the failure table returned for an attempt where the session did not come up, and `observed`, the four observations it was composed from. `_signin_of` whitelists the keys it resolves, so they are dropped on the way into the dossier rather than rendered — which is why this file says so out loud. A reader must not take this reader's silence for the log's: the observations are on disk from #325 onward and a log scan finds them. Surfacing them is a renderer decision this change deliberately did not take, because both renderers would have to agree on how a token is worded to the owner, and a verdict spoken carelessly is the failure the record exists to prevent.
