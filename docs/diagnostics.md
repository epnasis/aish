...
**A blob is written whole, through a temp file that is unique per CALL (#395).** A reader that finds a half-written blob cannot tell it from a complete one by looking, so `put` writes a scratch file beside the blob and renames it into place. The first version named the scratch after the PID, and aish-web runs every session on a thread pool inside ONE process: two triggered chats with an identical brief recorded the same digest in the same instant, both saw it absent, both wrote the one scratch path, the first rename consumed it and the second raised `FileNotFoundError` — out of `_record_brief`, so the whole task died. CI caught it twice on one commit; eight local runs never did. `atomic_write.publish` is now the only place a scratch name is made (`tempfile.mkstemp`, unique by construction), it removes the scratch on every failure path so no lister of the store ever meets one, and for a content-addressed store it yields to a concurrent winner: a blob already at the path holds these exact bytes, so the loser drops its scratch and the store is right either way. `turns.put` shares the fix, as do the two ledgers written whole on change (`roles.write_admission`, `vouches._write`). `TestPublish` pins the helper's own contract — two calls never share a scratch path, a winner-in-place is yielded to, a failed write or a refused rename leaves nothing behind; `TestNoTempFileIsKeyedOnThePid` sweeps `aish/` for the PID spelling, so the class is closed rather than the instance; `TestContentAddressedStoresUnderConcurrency` drives both stores from a barrier-aligned thread pool, which reproduced the CI failure six times in eight threads on the old code.
...
**`messages`** is this turn's message records, text included once, each with its `model_call` stamp as recorded (None where the writer wrote none). A model call's `context` block is computed from the stamps — which messages were `new` to it (stamped with the previous call), which were `in_front` of it, how many are `unstamped` and cannot be placed, which results a trim `stubbed` between the previous call and this one, and whether the brief changed at it — and is labelled `source: reconstructed` where it is built from records and the brief rather than from the request that left aish. **Since slice 2 it is labelled `source: sent` — with `ref.sent` naming the call — wherever `sent.calls` holds that call's manifest** (`CONTEXT_SENT` / `CONTEXT_RECONSTRUCTED` in `explain.py`): the request as it left aish exists, so a renderer shows the bytes by digest instead of the reconstruction. A call whose blobs were evicted or purged is still `sent` — the manifest says what was sent and which of it is gone, which is a different answer from a log with no manifest — and the reconstructed fields stay in the block either way, as the fallback for a document that cannot fetch. `TestTheStepList` pins both labels on the same records with and without the `sent` lines. `context_cost.calls[].by_where` gives the per-side totals (carried from earlier turns, this turn, the system text, the tool menu) for the same call, kept when the parts list is popped.
...
### Pre-flight recall is a step too (#386)

The "Recalled knowledge" row at the top of a card had nowhere to land: the `knowledge` record was folded into `given` and never into `steps`, so the row's tap was silent — the dead control L7 rules out. It is now a `knowledge` event step (`k<n>`, one `event` pane, placed before the first model call in file order) carrying the record's `items[]` with their per-item retrieval numbers (#183) and **the text the model was actually handed**. That text is the per-task system reminder (`agent.task_reminder`: the time note, the rules in force, the preloaded knowledge in one message) and it is on record only as one of the brief's system parts (#239) — by position and digest, unlabelled. `_reminder` locates it on the turn's own brief and **says how** (`located`), two joins in this order:

**By digest (#396, `located: brief_digest`).** Since #396 the `knowledge` record carries `reminder`, the `evidence.digest_of` of the very string `run_task` appended as the system message — the same content address `_system_evidence` gives the brief's parts — so the part whose digest equals the stamp IS the text, at whatever position it sits. The stamp is the writer's own word about which message it built, which is why a stamp matching NO part is its own state, **`not_on_brief`**, and never a cue to try position: the brief may well have the positional shape, so position would find a text, and that text shown as "what the model was handed" would be a guess wearing the stamp's authority. `not_on_brief` means *the two records disagree* — distinct from `not_located`, which means *the reader cannot tell* — and the pane says exactly that and names no cause (contract §3.8). `TestTheKnowledgeStep.test_two_extra_parts_resolve_by_digest_where_position_could_not` is the case the stamp exists for: a brief with two parts beside the standing prompt, which position must refuse and the digest resolves.

**By position (`located: brief_position`), only for a record with no stamp** — a log older than #396: the loop that wrote such logs kept exactly two system-role messages in front of a turn's first call, the standing prompt at position 0 and the reminder appended at seed with every earlier one stripped (since 2026-09-23 earlier reminders are kept, but every record written since carries the stamp, so this join never sees them), so the reminder is the one part not at position 0 of the brief written at model call 1. That is a fact about the writer, not the record, so the join REFUSES where the brief has a system half of the wrong shape — no part beside the standing prompt, or two, or two briefs for call 1 — as `not_located`, a fourth state beside `recorded`, `purged` and `not_recorded`. **`not_located` means "on record, unlocatable", so it is reserved for a brief that HAS parts**: the #239 stratum wrote briefs with no `system` half at all, and `_given` reports those as `system.state = not_recorded` — the step agrees and says `not_recorded`, with `why` telling the two not-recorded logs apart (`no_brief`: no brief for the first call — a log predating the record, or a turn cancelled before its first call; `system_not_kept`: a brief was written, its system text was not). The first cut folded the system-less brief into `not_located` and the pane printed "on record" about a document whose own `given.briefs[0].system.state` said otherwise.

**The role on the wire rides on the step** (`system_role`, from the brief's `options`): a `first_only` provider — gemini, openai, the default — relabels the reminder as a USER message (#74), so the pane says "sent as a USER message on this provider" there, "hoisted into the request's system parameter" on claude, and "system message" only on `all_system`; a step that hardcoded "system message" would describe an authority the model was never given. The pane shows the reminder WHOLE and labelled as the message (fidelity: never cut down to "the knowledge"), and in every other state shows no text at all: nothing is ever rebuilt from the item labels. `TestTheKnowledgeStep` drives the real seed path and pins that the step's text is byte-identical to the system message the fake backend received and that the record's stamp is that message's digest (`located == brief_digest`), the unstamped fixture resolving by position, the disagreeing fixture refusing as `not_on_brief`, the system-less brief, both roles, and the refusals. `tests/js/test_knowledge_step.js` pins each join's wording and that `not_on_brief` names no cause.
