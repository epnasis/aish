...
**The denominator is `backends.context_window`**, with its provenance in the tooltip: exact on Ollama (`num_ctx`) and `local:` (`AISH_LOCAL_CTX`), a conservative per-provider table figure on the cloud backends. It is not marked where trimming starts: except on `local:`, the trimmer budgets HISTORY in characters, with the system text and tool menu on top, so there is no single token line to draw. On `local:` it budgets the whole request in estimated tokens (#415, `docs/agent-core.md`), and each call's `reasoning` record carries that estimate as `prompt_estimate` beside the count.
