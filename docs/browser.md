...
Approval is per SITE and lasts the session (`_approved_sites`), matching the egress gate: a task that reads five pages of one portal asks once. It is session-scoped like every other grant (L4). With no approver it fails **closed** — reading the owner's account with nobody watching is the one outcome this exists to prevent.

**ONE PERMISSION, NOT TWO, AND THE QUESTION IS "MAY AISH READ THIS SITE AS YOU" (#287).** It used to be asked twice — once here for `read_url`, once at `_grant_host` for `browse` — on the reasoning that clicking is the bigger act and deserves its own answer. The owner rejected that, and the machine was already on his side. What the driving grant covered was a click that navigates or expands and a form that searches or filters, which is not "changing things" — it is *how you read a modern site*; while a control whose NAME says it pays, buys, books, deletes, sends or signs draws its own card regardless of any grant, a page showing checkout structure re-cards every submit, and passwords and the irreversible list are refused with no yes button at all. So the two tools were never two permissions. They were one permission described by implementation, and the split bought a second card and nothing else.

His argument for collapsing them is the one that decides it: *"whenever I see browsing I'll just automatically approve — the decision fatigue kicks in very quickly."* A distinction enforced only by a card he taps through is not enforced; it is theatre that costs attention which the cards that DO matter then have to compete with.

**The words moved with the merge, and they matter as much.** The old card said aish "may fill in and submit forms without asking again", which reads as a licence to CHANGE things — so the one clause it kept was the frightening one, while the bound it should have stated was the clause dropped for length. `SITE_GRANT` states the bound instead:

> browse eon.pl signed in as you — aish can read your private pages there; changing anything asks first.

That second clause earns its length precisely because it is the assumption he already makes when he approves. Better stated than smuggled. And the verb is **browse**, not *drive*: "drive" is a mechanism word, and mechanism words are how a card ends up describing the tool instead of the consequence.

**What the merge costs, said plainly.** The mutation floor is now the only fence between "I approved browsing" and an unwanted change — a word list plus escalating page evidence. It is decent and multilingual, and it will one day miss a button. That is the accepted trade, not an oversight: the split it replaced was not protecting him either. It makes two things load-bearing rather than nice-to-have — coverage of `says_it_commits`, and #281's digest, so a miss becomes visible after the fact instead of never.

**The site granted is the URL's own host, not the login record's key.** `is_logged_in` answers `google.com` for a Cloud blog post, because a bare `google.com` sits in `logins.txt`; granting THAT would have handed over `mail.google.com` because he read a blog. So `_login_gate` grants `browser.host_of(url)` — the site he was looking at — and `_site_granted` widens downward from there.

**And the grant survives the agent being rebuilt.** `_approved_sites` lives only in the Agent, and a chat outlives it: every ship restarts `aish-web` and builds a fresh agent under a chat still on screen, turning a yes given a minute ago into another card. Both gates write `kind:"site_grant"` and `SessionLog.site_grants` replays it into the SAME chat on reopen — never across chats, so L4 holds. It still reads the two older record names (`browse_grant`, `login_grant`), because chats written under them are still open.
