...
**The driving grant already covers reading the same site, and until #285 it did not.** He asked for a colleague's latest LinkedIn post, approved *drive linkedin.com AS YOU*, and forty-seven seconds later got a second card asking whether aish could **read** linkedin.com as him — strictly less than what he had just granted, since driving a site is what opens its pages and hands back their text. The two grants lived in separate sets (`_approved_browsing`, `_approved_logins`) and neither consulted the other, so what he experienced was aish asking the same question twice, and the only thing distinguishing the two cards was *which tool fetches the page* — a fact he has no way to see and nothing to do with. So `_login_gate` now passes when `_browse_grant_covers` says the URL is inside a site he already let aish drive, suffix-matched exactly like `is_logged_in`. The containment is **one-directional on purpose**: driving is the bigger question, and a read grant buys none of it.

**And it survives the agent being rebuilt, for the same reason the driving grant does.** `_approved_logins` lived only in the Agent, and a chat outlives it: every ship restarts `aish-web` and builds a fresh agent under a chat still on screen, turning a yes given a minute ago into another card. `_login_gate` now writes `kind:"login_grant"` and `SessionLog.login_grants` replays it into the SAME chat on reopen — never across chats, so L4 holds.
