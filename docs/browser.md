...
**May aish use this site as him at all** — asked at the first PRESS, once per site, session-scoped like every other grant (L4), because a card per click is a card nobody reads and a flow through one portal is twenty clicks.
...
**Both value checks are BATCH-ONLY, and that is a hole, not a design (#310).** They live in `_irreversible_step`, which only the `browse_fill` path reaches. `browse_act` takes `action="type"` with a `text` of its own and its branch tests two things — a password field, and an irreversible **label** — so the same value, on the same page, is refused by one tool and typed by the other. This is the failure this file already names against the site grant one section down (*"the model chooses the tool, which made the card bypassable for exactly the half it was covering"*), and until #310 moves the checks onto the act of typing rather than onto the tool doing it, "aish cannot type a card number" is true of a batch and not of aish.
...
**So the refusal has to name what it saw.** `BROWSE_NO_CARD_NUMBER` says which step it stopped at, that the value looked like a card number, and that no approval changes it — and says the same thing whether or not the value really was a card, since the answer does not depend on that. What it never carries is the digits. A refusal that names nothing teaches nothing; a refusal that names the value defeats itself.
...
So `_fence_the_origin` routes the page's requests for the duration of the sign-in and **aborts anything with a body going to another origin**. That needs no guess about encoding — it never goes looking for the password in a body that may be JSON, form-encoded or percent-escaped — and it catches the real-form case too. Cross-origin **GET**s are untouched, because fonts, images and scripts are how the login page renders at all. A blocked request is not shrugged off: the sign-in stops and says where the page tried to send something, and the result is never reported as a success.
