...
**What this does not fix.** `_press`'s last rung still dispatches a click event straight at the control and says the page *may* not have registered it, when it very often has — a guess where a readback is possible. `fill` reads its fields back; a single `browse_act` does not.
