...
- Sites he really was signed into were missing, and then the two doors disagreed: `read_url` fetched the page anonymously and handed back the signed-out version as his account, while `browse` drove the same site as him after one generic *may I drive this host* card, because the driving gate never consulted the record at all.
...
**The read path now escalates on the PAGE, not on the list.** A login wall is not a failure — it is 200 with a full page of text — so nothing on that path escalated for it, which is why the list was load-bearing at all. `browser.asks_to_sign_in` already read a password field out of fetched HTML and was used only to word a note; it is now the trigger. Fetch, and if the fetch meets a door, render it in his browser.

**What survives is a HINT, and the distinction is the whole design.** `seen_signed_in()` holds hosts aish has *watched* a sign-in succeed at — a rendered page that stopped asking for a password, a credential replay that worked, a sign-in typed in the remote view. Every entry traces to an event aish saw. It exists only to skip the doomed anonymous fetch, **nothing gates on it, and no claim to the owner rests on it**: a wrong entry costs one wasted Chrome launch, and the very next read corrects it in either direction (`note_signed_in` / `note_signed_out`). It is matched **exactly** rather than by suffix, unlike the record it replaces: a claim about an *account* reasonably covers a whole site, an observation covers the page it was made on — and since being wrong now costs a launch instead of a false statement, there is nothing to be lenient for. A subdomain simply meets its own wall and escalates. `TestTheHintIsWrittenByObservation`.

**The read-time approval card is gone with the list.** Its question was *"does this read carry your live session?"*, and the honest answer is that he had already answered it — at the sign-in itself, with his hands, on the site, knowing exactly what he was doing. Re-asking afterwards was a worse version of a decision already made, delivered into the channel he has said plainly he does not read: *"there's this decision fatigue if I have to please click approve for every action. It's not gonna happen."* A card tapped blind is worse than no card, because it records a consent he never gave.

What enforces anything instead is where it belonged all along: the taint fence stops the data leaving (`docs/agent-core.md`), the irreversible list refuses the acts that cannot be undone, and reading his own account — the thing he asked for — is free and *reported*. The provenance note above the untrusted-content banner still says which identity a page was read with, because the model must be able to tell his account from a stranger's view of it.

**Driving keeps its card**, and it always asked the better question anyway: *drive eon.pl in your signed-in browser — aish will open pages and click on them AS YOU*. One card per host per task, and the consequential presses inside it keep their own.
...
That narrowing shipped and was itself superseded within the day: once the answer is read off the page there is nothing left to ask at all, and closing the view now REPORTS what it watched instead of asking for a claim. Kept here because the sequence is the lesson — three narrowings of a question, then the realisation that the question was the problem.
