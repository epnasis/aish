...
**The delimiter rule is the web's, mirrored, and the two move together.** `_MATH_RE` is the Python spelling of `app.js`'s `MATH_RE` plus its `findMath` checks: `$$…$$`, `\[…\]`, `\(…\)` as they come, a single-`$` span only under the conservative rule that keeps `it costs $5 and the other is $10` prose (the clauses are listed in `docs/web-frontend.md`). `tests/test_export.py` pins the SAME case lists `tests/js/test_math_render.js` does, and the same fixture — the real answer that filed the issue, byte-identical to the owner's log: every `$` consumed, 13 display + 38 inline spans. It runs as an inline pattern after `backtick` (a `$` in a code span is never seen) and before the escape and emphasis patterns (the `_` and `*` inside an equation are never eaten).
