...
**Bounded.** Typesetting is superlinear in the source length (1k chars 0.09 s, 8k 1.2 s, 16k 3.2 s, a reviewer's 50k 146 s and a 44 MB SVG), `EXPORT_MAX_BYTES` admits 5 MB, and every export tap stacks another thread. A span over `_MATH_MAX_TEX` (1000 chars — eight times the real answer's longest, 128) is refused before either library sees it and printed verbatim. `test_a_span_over_the_cap_is_verbatim_in_bounded_time`.
